...
public class ControladorBattleship {
    ...
    public void iniciar() {
        ...
        if (ui instanceof ConsolaUI) {
            ((ConsolaUI)ui).close();
        } else if (ui instanceof VentanaUI) {
            ((VentanaUI)ui).close();
        }
    }
    ...
    private void iniciarJuego() {
        ...
        ui.mostrarMensaje("Juego finalizado. Presiona Enter para cerrar la conexión.");
    }
    ...
    private boolean turnoLocal() throws IOException {
        ...
        try {
            ...
            // Volver a mostrar el tablero enemigo actualizado
            ui.mostrarTablero(juego.getTableroEnemigoGrid(), "TABLERO ENEMIGO");
            ...
            switch (mensaje.comando) {
                ...
                case ManejadorMensajes.JUEGO_TERMINADO:
                    // El oponente confirmó que has ganado
    ...
    private int[] obtenerDisparoSeguro() {
        ...
        do {
            // ConsolaUI/VentanaUI se encarga de obtener la entrada válida (0-9)
            disparo = ui.obtenerDisparoJugador(juego.getTableroEnemigoGrid()[0][0] != '?');
    ...
    private boolean turnoRemoto() throws IOException {
        ...
        try {
            ...
            if (ManejadorMensajes.DISPARAR.equals(mensaje.comando)) {
                ...
                // Obtener el tipo de barco ANTES de marcar 'X' en el tablero propio
                String tipoBarcoImpactado = juego.obtenerTipoBarcoEn(mensaje.x, mensaje.y);
                boolean impacto = juego.procesarDisparoOponente(mensaje.x, mensaje.y);
                
                if (impacto) {
                    if (juego.estaBarcoHundido(tipoBarcoImpactado)) {
                        // Hundido
                        conexion.enviarMensaje(ManejadorMensajes.construirMensajeResultado(
                                ManejadorMensajes.HUNDIDO, mensaje.x, mensaje.y, tipoBarcoImpactado));

                        ui.mostrarMensaje("El oponente hundió tu " + tipoBarcoImpactado + " en (" + mensaje.x + "," + mensaje.y + ")");

                        if (juego.todosBarcosHundidos()) {
                            // Hemos perdido, enviamos la señal de victoria al oponente
    ...
    public static void main(String[] args) {
        ControladorBattleship juego = new ControladorBattleship();
        juego.iniciar();
